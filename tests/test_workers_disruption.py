...
def _make_segment(
    seg_id: str, title: str, start: datetime, end: datetime, seg_type=SegmentType.FLIGHT
) -> TripSegment:
    return TripSegment(
        id=seg_id,
        type=seg_type,
...
def test_detect_downstream_collisions_clean_schedule():
    ...
    seg1 = _make_segment(
        "seg_1", "Leg 1: JFK -> LHR", now, now + timedelta(hours=7)
    )  # 08:00 - 15:00
    seg2 = _make_segment(
        "seg_2", "Leg 2: LHR -> CDG", now + timedelta(hours=9), now + timedelta(hours=10)
    )  # 17:00 - 18:00 (120m buffer)
...
def test_detect_downstream_collisions_insufficient_buffer():
    ...
    seg1_delayed = _make_segment(
        "seg_1", "Leg 1 (Delayed)", now, now + timedelta(hours=8, minutes=30)
    )  # 08:00 - 16:30
    seg2 = _make_segment(
        "seg_2", "Leg 2: LHR -> CDG", now + timedelta(hours=9), now + timedelta(hours=10)
    )  # 17:00 - 18:00 (30m gap vs 90m req)
...
def test_detect_downstream_collisions_direct_overlap():
    ...
    seg1_severe = _make_segment(
        "seg_1", "Leg 1 (Severely Delayed)", now, now + timedelta(hours=9, minutes=45)
    )  # 17:45
    seg2 = _make_segment(
        "seg_2", "Leg 2: LHR -> CDG", now + timedelta(hours=9), now + timedelta(hours=10)
    )  # 17:00
...
def test_disruption_reasoning_worker_generates_pending_actions():
    ...
    now = datetime(2026, 9, 15, 8, 0)
    seg1 = _make_segment(
        "seg_1", "Leg 1", now, now + timedelta(hours=9, minutes=30)
    )  # arrives 17:30
    seg2 = _make_segment(
        "seg_2", "Leg 2", now + timedelta(hours=9), now + timedelta(hours=10)
    )  # departs 17:00
