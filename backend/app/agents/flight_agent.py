...
STANDALONE_FLIGHT_REGEX = re.compile(
    r"\b([A-Za-z]{2,3})\s*(\d{1,4}[A-Za-z]?)\b"
)
    ...
    def _extract_flight_params(self, state: TravelState) -> Dict[str, Any]:
        ...
        if not params.get("flight_iata") and not params.get("flight_number"):
            ...
            if explicit_match:
                ...
                logger.info("Extracted flight identifier from explicit phrasing: %s", params.get("flight_iata") or params.get("flight_number"))
            else:
                ...
                if standalone_match:
                    ...
                    if code not in STOPWORDS and not is_year:
                        ...
                        logger.info("Extracted standalone flight identifier: %s", params["flight_iata"])
    ...
    async def execute(self, state: TravelState) -> Dict[str, Any]:
        ...
        if intent != "flight":
            ...
            return {
                ...
                "errors": current_errors + [f"Flight Agent invoked with invalid intent '{intent}'. Expected 'flight'."],
            }
        ...
        try:
            logger.info("Querying AviationStack with parameters: %s", {k: v for k, v in flight_params.items() if not k.startswith("raw_")})
        ...
        normalized_data: List[Dict[str, Any]] = [
            f.model_dump() if isinstance(f, NormalizedFlight) else dict(f)
            for f in response.data
        ]

        status = "success" if normalized_data else "no_results"
        logger.info("Flight Agent completed lookup (results=%d, status=%s)", len(normalized_data), status)
