...
class RouterDecision(BaseModel):
    ...
    intent: AllowedIntent = Field(description="The selected operational travel intent.")
...
class RouterAgent:
    ...
    def route(self, state: TravelState) -> Dict[str, Any]:
        ...
        if not user_query or not isinstance(user_query, str) or not user_query.strip():
            logger.warning(
                "Router received empty or missing user query; classifying as unsupported."
            )
            return {
                "intent": "unsupported",
        ...
        if isinstance(response, RouterDecision):
            ...
            try:
                ...
                logger.warning(
                    "Router model returned invalid intent dictionary %r: %s", response, exc
                )
                current_errors = list(state.get("errors", [])) if isinstance(state, dict) else []
                return {
                    "intent": "unsupported",
                    ...
                    + [f"Invalid intent '{invalid_intent}'; fell back to unsupported."],
                }
        ...
        if decision.intent not in ALLOWED_INTENTS:
            ...
            return {
                ...
                "active_agent": "router_agent",
                "errors": current_errors
                + [f"Unrecognized intent '{decision.intent}'; fell back to unsupported."],
            }
    ...
    async def aroute(self, state: TravelState) -> Dict[str, Any]:
        ...
        if decision is None or decision.intent not in ALLOWED_INTENTS:
            ...
            return {
                ...
                "active_agent": "router_agent",
                "errors": current_errors
                + [f"Unrecognized intent '{unrec}'; fell back to unsupported."],
            }
